...
const findBorrows = async (client?: Client) => {
  const databaseClient = client ?? (await initializeDatabaseClient());
  const borrowRes = await databaseClient.query('SELECT * FROM booking');
...
const findBorrowByBookId = async (book_id: string, client?: Client) => {
  ...
  const borrowRes = await databaseClient.query(
    'SELECT * FROM booking WHERE book_id = $1 LIMIT 1',
    [book_id]
  );
  if (!client) databaseClient.end();
...
const extendBorrow = async (id: string, client?: Client) => {
  ...
  const databaseClient = client ?? (await initializeDatabaseClient());
  const borrowRes = await databaseClient.query(
    'UPDATE booking SET return_date = $1 WHERE id = $2 RETURNING *',
    [return_date, id]
  );
  if (!client) databaseClient.end();
  return borrowRes.rows[0];
