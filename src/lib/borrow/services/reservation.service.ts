...
const findReservationByBookId = async (book_id: string, client?: Client) => {
    ...
    const reservationRes = await databaseClient.query('SELECT * FROM reservations WHERE book_id = $1 LIMIT 1', [book_id]);
...
const findReservationsByUserId = async (user_id: string, client?: Client) => {
    const databaseClient = client ?? (await initializeDatabaseClient());
    const reservationRes = await databaseClient.query('SELECT * FROM reservations WHERE user_id = $1', [user_id]);
    if (!client) databaseClient.end();
    return reservationRes.rows;
}

const createReservation = async (book_id: string, user_id: string, client?: Client) => {
    ...
    const databaseClient = client ?? (await initializeDatabaseClient());
    const reservationRes = await databaseClient.query('INSERT INTO reservations (book_id, user_id, reservation_span) VALUES ($1, $2, $3) RETURNING *', [book_id, user_id, reservationSpanStr]);
    if(!client) databaseClient.end();
    return reservationRes.rows[0];
}
...
const deleteReservation = async (id: string, client?: Client) => {
    const databaseClient = client ?? (await initializeDatabaseClient());
    const reservationRes = await databaseClient.query('DELETE FROM reservations WHERE id = $1', [id]);
    if(!client) databaseClient.end();
    return reservationRes.rows[0];
...
export { findReservationByBookId, createReservation, deleteReservation, findReservationsByUserId };